...
lake_loc = {'lake_one': {'lake_pos': (1300, 1000, 1300 + 1500, 1000 + 1800),
            ...
            'lake_two': {'lake_pos': (4000, 1800, 4000 + 1200, 1800 + 1200),
                         'special_pos': [[4400, 2200], [4400, 2500], [4400, 2700],
                                         [4700, 2200], [4200, 2400], [4550, 2400]]},
            'lake_three': {'lake_pos': (4800, 2500, 4800 + 1700, 2500 + 1590),
                           'special_pos': [[5500, 3900], [5500, 3600], [5700, 3600],
                                           [5800, 3300], [6100, 3000], [5100, 3800],
                                           [4850, 3800], [5700, 3900], [5300, 3600]]}}
...
def fine_tune(x1, x2, y1, y2, img, mask, special_pos, visualize):
    ...
    epochs = 3
    if visualize:
        fig, axs = plt.subplots(1, epochs + 1, constrained_layout=True, figsize=(20, 4))
    ...
    for j in range(epochs):
        ...
        threshold_array = k_means(threshold_array, n_clusters=10)
        # threed_color_visualize(threshold_array)
        ...
        for i in tqdm(range(len(threshold_array))):
            ...
            bottomk = int(len(img_distance) / len(threshold_array) / 10)
        ...
        if visualize:
            ...
            axs[j + 1].set_yticks([])
    if visualize:
        fig.savefig(f'{plot_directory}stage_two.pdf')
        plt.show()
    mask[x1:x2, y1:y2] = 1.0 - img_one_idx
    return mask


def stage_two(mask, img, date, visualize):
